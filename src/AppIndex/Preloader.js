...
const getColor = (line, cycle, offsetLine, fillStyle) => {
  const horizontalLine = 1 - Math.abs(line) / offsetLine;
  if (horizontalLine > 0) {
    const currentColor = fillStyle[cycle % 2 ? 'first' : 'second']
      .replace(/[^\d,]/g, '')
      .split(',');
    const secondColor = fillStyle[cycle % 2 ? 'second' : 'first']
      .replace(/[^\d,]/g, '')
      .split(',');
    const color = calculateColor(
      horizontalLine,
      currentColor.map(Number),
      secondColor.map(Number),
    );
...
const Preloader = ({
  ...
  const render = (time) => {
    ...
    pathOffsets.forEach((dot) => {
      ...
      dot.path.style.transform = `scale(${heightOffsetCalc(
        differenceSize,
        dotScaleSize,
        offsetLine,
        dot.offsetY,
      )}) translateX(${
        dot.offsetX > 8 || dot.offsetX < -8
          ? offsetCalc(
              differenceSize,
              dot.offsetX,
              offsetLine,
    ...
    const newOffset =
      (((new Date().getTime() - startTime) / customDuration) * endPos) % endPos;
